...
if [ -f best.pt ]; then
    ...
    huggingface-cli download UItraviolet/yolo_multicart runs/segment/train-2/weights/best.pt --local-dir temp_weights --local-dir-use-symlinks False
...
if [ -d "$local_dataset_dir" ] && ls "$local_dataset_dir"/*.parquet &>/dev/null; then
    ...
    huggingface-cli download UItraviolet/yolo_multicart --repo-type dataset --include "*.parquet" --local-dir "$local_dataset_dir" --local-dir-use-symlinks False
