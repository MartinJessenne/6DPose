...
class TestConstrainedRansac(unittest.TestCase):
    ...
    def test_z_gate_rejects_inconsistent_frame(self):
        ...
        result = constrained_ransac_se2(
            model_points, scene_points, model_fpfh, scene_fpfh,
            distance_threshold=0.01, max_iterations=500, z_offset=0.0, rng=rng,
        )
    ...
    def test_seeded_runs_are_reproducible(self):
        ...
        for _ in range(2):
            ...
            result = constrained_ransac_se2(
                model_points, scene_points, model_fpfh, scene_fpfh,
                distance_threshold=0.01, max_iterations=1000,
                z_offset=z_offset, rng=rng,
            )
            results.append(result)
        np.testing.assert_array_equal(
            results[0].transformation, results[1].transformation
        )
    ...
    def test_too_few_correspondences_returns_failure(self):
        ...
        feats = np.ones((1, 33))
        result = constrained_ransac_se2(
            model_points, scene_points, feats, feats.copy(),
            distance_threshold=0.01, max_iterations=10,
        )
        self.assertEqual(result.fitness, 0.0)
...
class TestSe2Icp(unittest.TestCase):
    ...
    def test_converges_from_perturbed_init(self):
        ...
        result = icp_point_to_plane_se2(
            ...
            max_correspondence_distance=0.3, max_iterations=50,
        )
    ...
    def test_no_correspondences_returns_init(self):
        ...
        result = icp_point_to_plane_se2(
            model_points, scene_points, scene_normals, T_init,
            max_correspondence_distance=0.05, max_iterations=20,
        )
        self.assertEqual(result.fitness, 0.0)
    ...
    def test_dual_hypothesis_recovers_flipped_init(self):
        ...
        result = refine_pose_dual_hypothesis_se2(
            model_points, scene_points, scene_normals, T_init_flipped,
            max_correspondence_distance=0.3, max_iterations=50,
        )
