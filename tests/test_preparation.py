...
class TestEstimatorPreparation(unittest.TestCase):
    ...
    def test_ppf_cache_hit_and_parameter_isolation(self):
        ...
        est2 = PPFEstimator(params=PPFParams(ppf_sampling_step=0.05, ppf_distance_step=0.02, icp_max_iterations=100))
        est2.prepare(self.mesh, "test_cart")
    ...
    def test_copy_on_retrieval_and_mutation_safety(self):
        ...
        cached_pc_initial = np.asarray(BasePoseEstimator._PREPARATION_CACHE[cache_key]["model_pc"].points).copy()
        ...
        cached_pc_after = np.asarray(BasePoseEstimator._PREPARATION_CACHE[cache_key]["model_pc"].points)
